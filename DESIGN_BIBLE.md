...
Prototype resources and inventory have lab-wide totals backed by physical item stacks and compatibility room ledgers. The global number remains a readability ledger, but each stack records its actual tile, room, quantity, unit mass and volume, containing fixture or floor location, stockpile designation, and last observed quantity. Physical availability depends on where the stack is and whether an actor can reach its storage fixture. Waste still defaults to the Pits because it is a problem to manage, not a clean supply shelf.
...
Deleting a room designation does not delete or teleport its stockpile. Its physical stacks remain on their existing tiles and stay part of the lab-wide ledger. If another appropriate stockpile designation exists, the scientist can automatically haul them there; otherwise the supplies remain where they were. Richer loose-item interaction belongs to the later tile-contents and labor passes.

Rooms do not have an abstract capacity limit. Storage furniture enforces its own volume and mass limits, while designated floor storage has no tile-capacity limit in this pass. The loose-items pass should replace that temporary floor behavior with physical crowding, stacking, spills, and richer storage quality.
