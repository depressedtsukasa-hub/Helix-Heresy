...
Room stockpile UI is framed as last-inventoried knowledge rather than perfect omniscience. Prototype counts still update exactly for now, but the player-facing language should support future interference from creatures, incidents, raids, spills, theft, fires, or inaccessible rooms. The Stores workspace now has tabs for Overview, Materials, Byproducts, Specimen Materials, Tools & Supplies, Room Stockpiles, Collection Stations, and the temporary Scientist sheet. Global known totals and room-local breakdowns are shown together, but physical actions still care where stock is stored. Selecting a room or in-room tile shows Known Supplies in the Selection inspector, including resources, tools, collected byproducts, harvested specimen materials, and pit contents where relevant. Tools appear in the same physical room supply readout as materials because they are stored objects, not a magical equipment cloud. Waste inside pit containers or the Pits room is displayed as pit contents rather than clean inventory.

Deleting a room designation does not delete or teleport its stockpile. The stock becomes a floor pile at the former room anchor and remains part of the lab-wide ledger. If another appropriate stockpile zone exists it is recorded as the intended hauling destination; otherwise the supplies remain where they were. Actual autonomous hauling and richer loose-item interaction belong to the later physical stockpile and labor passes.
...
Collection Bay transfer is also queued. The scientist closes the station path, swaps the active receptacle, carries the collected material to the Storage Room ledger, and leaves any apparatus overflow in the Collection Bay to flow into the replacement receptacle. The Stores Collection Stations tab is a read-only station ledger showing each station's active receptacle, overflow buffer, output, method, and support. Transfer Receptacle lives on the selected station container's contextual commands, keeping the record tab informational while the map selection remains the action surface. Overflow remains apparatus contents rather than collected inventory.
...
Room designation policy has Off, Suggest, and Automatic modes. Off leaves inferred geometry available without creating rooms. Suggest exposes the inference for confirmation. Automatic accepts unambiguous inferred areas, can extend non-manual room designations into matching unassigned floor, and assigns an obvious purpose when defining contents provide exactly one answer. It never changes manually drawn boundaries or overwrites an already chosen purpose. Automatic purpose assignments record their rationale, such as synthesis equipment, a collection station, a processing pit, or stocked materials. Ambiguous contents remain unassigned.
...
Construction now begins from the physical blueprint. The player enters Dig Mode, clicks solid-earth tiles on the map to build a draft designation, then confirms that draft into a timed Excavation task. Draft cells are saved as tile sets rather than only rectangles, so irregular shapes are supported by the same walkable-cell model used by pathfinding. Invalid drafts, such as disconnected cells, overlap, oversized drafts, or drafts that do not touch existing excavated floor, are shown as draft problems before work can be queued. Planned dig tiles are marked on the blueprint. When the task completes, the designated tiles become rough excavated floor; excavation itself does not manufacture a room record or a door. Compartment inference then recalculates, and the room policy may accept an unambiguous area. Selecting rough floor or a room exposes inferred acceptance, manual room drawing, room-purpose commands, and the Rooms overlay through the contextual command surface. Current purposes include Workroom, Containment Room, Corpse Processing Room, Quarters, Storage Room, Collection Room, and Corridor. This keeps the Dwarf Fortress-style order of operations: dig space first, recognize or draw room ownership, optionally construct boundaries, then define what the room is for.

Future construction, room expansion, damage, sealing, ventilation, drainage, power, and creature movement should extend this physical map model. Detailed room interaction now starts from map selection and contextual commands rather than always-visible room cards; room stockpiles, station records, policies, tasks, and historical logs live in their management menus. This first construction pass does not yet model excavation materials, money, noise, secrecy, workers, supports, room equipment, or buildable infrastructure; those should be layered on top of tile designation and room-purpose assignment rather than replacing them.
