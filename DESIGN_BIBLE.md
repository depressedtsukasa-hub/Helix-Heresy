...
Inferred compartments are conveniences, not final authority. The Rooms overlay shows inferred areas, corridor-like spaces, thresholds, compromised boundaries, existing room ownership, and the current manual room draft. Players can accept an inferred compartment or draw any contiguous excavated tile area as a room, including dividing one inferred compartment into multiple persistent rooms. Formal room tile ownership is exclusive. If a manual redraw leaves the remainder of an existing designation disconnected, both sides remain owned by that room and the designation is flagged for later correction instead of silently deleting or reassigning tiles. Door-adjacent ownership changes are intentionally deferred until dynamic door placement can relink doors safely.
...
Spatial environment propagation has a room-level contamination diffusion foundation driven by physical openings. Contamination behaves like a concentration gradient rather than a scripted hazard timer: higher-contamination areas leak toward lower-contamination areas over time, scaled by room volume, elapsed time, and the physical connection. Open floor passages exchange freely. Open and breached doors allow far more exchange than closed doors, while closed doors leak according to effective seal quality. Sealed doors block creature/scientist passage but still allow tiny or meaningful contamination leakage depending on door type, condition, and seal-related wards. This pass intentionally covers contamination only; temperature, moisture, fumes, electrical charge, ambient mana, vents, drains, construction materials, and tile-level diffusion should build on the same model later.
