...
Released slimes now act as loose map actors. They are not added to the normal task queue; instead, each loose slime can choose a simple creature activity such as wandering, seeking accessible mess, moving toward contamination, feeding on room residue, feeding on loose waste, or feeding on accessible remains. They use the physical path map and their derived movement speed, and they can only cross open or breached doors. Closed, locked, and sealed doors block them. When a slime presses against a blocked route, the scientist records possible intent rather than perfect knowledge. Clean Storage Room supplies are treated as packaged or secured and are not free food for loose slimes in this pass.
