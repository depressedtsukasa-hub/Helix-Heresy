...
Map-based incident alerts are now a lightweight spatial emergency layer. Active alerts can be created for observed loose creatures, blocked-door pressure, notable room contamination, feeding residue spills, exposed remains, corpse overflow, compromised or breached containers, breached doors, and observed combat. Alerts store type, severity, source, room, map cell, status, acknowledgement timing, response movement timing, and urgency timing; they highlight the lab blueprint and appear in an Incident Alerts panel. The current response pass lets the player acknowledge an incident, focus its map source, or queue a scientist movement task to the alert site. New serious non-combat incidents reduce time to 1x, and new critical non-combat incidents pause at 1x when discovered. The response system deliberately does not auto-resolve the underlying problem yet: recapture, cleanup, repair, feeding, calming, fighting, sealing, and disposal still use existing or future systems.
...
The first map-first shell pass keeps the old prototype panels reachable while shifting visual hierarchy toward the blueprint. The Rooms/Blueprint panel now spans the workspace first, the main shell has a workspace navigator for Map, Foundry, Specimens, Containers, Resources, Policies, Journal, Log, and Cheats, and selected map targets can move the player to the relevant section. This is intentionally a transitional shell rather than the final screen manager: inactive sections remain visible for now so existing workflows and tests stay usable while later inspector and contextual-command passes migrate actions out of permanent panels.

Selection now has a single forward-facing state object that can represent tiles, rooms, doors, containers, living specimens, corpses, incidents, tasks, tools, or resources. The older `selectedSlimeId` and `selectedMapTarget` values are kept as compatibility mirrors while the prototype panels still exist. A temporary Selection inspector near the blueprint shows the current target, compact status, location, related entities, and other selectable things sharing the same map cell. Map clicks use top-priority selection for stacked cells; related links expose contained specimens or other same-location entities without interrupting normal clicks. If a selected slime dies and creates a corpse, selection follows the corpse automatically.

The first contextual command pass adds a reusable command definition and rendering layer to that Selection inspector. Commands are grouped by purpose, stay visible when disabled, and explain blocked requirements such as stamina, missing containers, route problems, security states, tools, or risk. Current first-pass commands cover selected room or tile movement, door open/close/lock/seal controls, incident focus/response/acknowledge actions, corpse necropsy/forensic/harvest/dump actions, slime Analyze/handling/combat/living-harvest actions, and container access/hauling/remains/transfer actions. Permanent prototype panels still exist, but new player actions should increasingly flow through reusable command functions so hotkeys, command modes, inspectors, and future map workflows have a shared foundation.
