...
Spatial environment propagation is owned by inferred physical compartments rather than room labels. Room attribute values remain player-facing weighted summaries, while recovery, container exchange, environmental sustenance, and contamination diffusion read and write the physical compartment containing the source. Compartment records migrate by tile overlap when topology changes: a split inherits its former conditions and a physical merge uses area-weighted conditions. Renaming, splitting, merging, or deleting room designations does not move air or erase hazards. Contamination behaves like a concentration gradient rather than a scripted hazard timer: higher-contamination areas leak toward lower-contamination areas over time, scaled by compartment volume, elapsed time, and the physical portal. Open thresholds exchange freely. Open and breached doors allow far more exchange than closed doors, while closed doors leak according to effective seal quality. Sealed doors block creature/scientist passage but still allow tiny or meaningful contamination leakage depending on door type, condition, and seal-related wards. This pass intentionally covers contamination only; temperature, moisture, fumes, electrical charge, ambient mana, vents, drains, construction materials, and tile-level diffusion should build on the same model later.
...
The starter lab includes a physical workbench, bed, collection apparatus, wall lamp, wall vent, floor drain, and Concealed Exit for testing. Buildable first-pass fixtures include the workbench, bed, collection apparatus, lamp, vent, and drain. The player can choose an exact material variant, the best known available material, or the closest available material, and can rotate a placement before confirmation. Inaccessible placement is legal because designations express intent rather than guaranteeing wisdom; the fixture remains impaired or unusable until at least one interaction port can be reached. Heavy site-built equipment cannot be hauled as an ordinary object. Moving it will require dismantling and reconstruction, while small portable furnishings can eventually be carried as finished items.

Fixture assembly is divided into portable finished items, workstation-built components assembled on site, and very large equipment built directly on site. A placement that requires a finished item or components automatically creates a linked production-bill record when that dependency does not exist. The current prototype deliberately leaves those bills blocked and provides a debug fulfillment command because loose finished items, workstation recipes, and autonomous crafting have not been implemented yet. Physical stockpiles are now implemented and provide the destination model those outputs will use. After a dependency is fulfilled, the normal scientist construction task claims the placement. The later crafting pass should turn the same dependency record into real bills rather than replacing this relationship. Containers and the Synthesis Tube still use specialized records but already follow the same physical ideas of footprint, access, material condition, and non-magical movement; they should migrate through adapters as the shared equipment system expands.
...
Player-facing AI readouts remain broad and readable, such as `AI: Moving - seeking loose biomatter` or `AI: Blocked - blocked from Loose biomatter`. The selected slime card includes an Activity panel that explains broad state, intent, target, urgency, observed pressures, path status, next decision timing, and unknown-factor warnings without exposing exact hidden weights. The Debug workspace contains a Slime AI Debug tab for the currently selected slime; this development-only view exposes raw AI state, intent, target data, path length, drive bands, perception entries, response scores, combat decision scores, social context, room activity, movement record, and containment testing state. Normal player UI should stay qualitative, while debug readouts can be exact enough to diagnose stuck pathing, invalid targets, and unexpected behavior.
...
- Tabbed Policies workspace for automation rules, including Overview, Handling, Corpses, Doors, Feeding, and Automation sections.
