...
The keyboard-control pass adds a map cursor, explicit navigation and command modes, and letter-driven management paths inspired by Dwarf Fortress. Navigation mode lets the player move the cursor with arrow keys, select the top-priority target under it with Enter, open contextual commands with `A`, clear selection with Escape, and toggle a help legend with `?`. Command mode displays numbered hints on the selected target's contextual commands; `1` through `9` activate those visible commands and Escape returns to navigation. Number keys are reserved for command mode rather than speed control. Time controls remain global from both Map and management menus: Space pauses, `>` increases speed, `<` decreases speed, `.` skips to the next meaningful event, and `Ctrl+.` skips to the next scientist queue completion. Top-level letter paths open management surfaces, such as `T` for Tasks, `S` for Stores, `C` for Creatures, `P` for Policies, `R` for Records, and `D` for Debug when visible. Submenu letters choose current tabs, such as `T Q` for Tasks > Queue, `S B` for Stores > Byproducts, and `P F` for Policies > Feeding. Tab remains native browser focus behavior for now rather than a game command.
...
- Debug Cheats tab with XP, resource, room, and inventory cheat commands for testing skill progression, resource-gated systems, room conditions, and inventory state.
...
- Keyboard shortcuts: Space pauses, > increases speed, < decreases speed, . skips to the next meaningful event, Ctrl+. skips to the next scientist queued task, O cycles map overlays, and Shift+O cycles overlays backward. Number keys are reserved for visible contextual commands while in command mode. Letter paths open management menus and tabs, such as T Q for Tasks > Queue, S B for Stores > Byproducts, C C for Containers, P F for Policies > Feeding, and R M for Records > Messages.
