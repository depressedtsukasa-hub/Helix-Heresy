...
Fixture assembly is divided into portable finished items, workstation-built components assembled on site, and very large equipment built directly on site. A placement that requires a finished item or components automatically creates a linked production-bill record when that dependency does not exist. The current prototype deliberately leaves those bills blocked and provides a debug fulfillment command because physical stockpiles, loose finished items, workstation recipes, and autonomous crafting have not been implemented yet. After that dependency is fulfilled, the normal scientist construction task claims the placement. The later crafting pass should turn the same dependency record into real bills rather than replacing this relationship. Containers and the Synthesis Tube still use specialized records but already follow the same physical ideas of footprint, access, material condition, and non-magical movement; they should migrate through adapters as the shared equipment system expands.
