...
Collection Bay transfer is also queued. The scientist closes the station path, swaps the active receptacle, carries the collected material to the Storage Room ledger, and leaves any apparatus overflow in the Collection Bay to flow into the replacement receptacle. The Stores Collection Stations tab shows each station's active receptacle, overflow buffer, output, method, support, and transfer action, but overflow remains apparatus contents rather than collected inventory.
...
The full-screen map shell now treats the blueprint as the base play surface. The map remains mounted behind the interface, the HUD keeps only compact run-state information such as time, speed, pause state, and Suspicion, and storage or waste capacity lives in management views rather than the always-visible status strip. Existing prototype screens are still reachable through the categorized workspace menu, but non-map screens open as temporary management overlays instead of permanent side panels. Time continues to run while those menus are open unless the game is already paused. This is still a transitional shell: the temporary Selection inspector, command actions, and legacy management screens will be migrated into more focused map-driven menus in later passes.
