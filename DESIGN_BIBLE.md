...
Collected byproducts move into storage through a queued station-level Transfer Receptacle task. Transfer requires an appropriate empty replacement vessel: the scientist closes the station path, removes the filled physical receptacle, installs the replacement, and hauls the filled vessel toward storage. Without a replacement, transfer remains blocked. Apparatus overflow is not collected during the swap; it remains in the Collection Bay and flows into the newly installed receptacle afterward. Raw byproduct names are preserved until a later processing system changes them, so smoke vapor stays smoke vapor unless it is run through a condenser or another refining procedure later. Routine transfer accounting belongs in inventory history and tooltips, while the event log should remain for meaningful observations, incidents, and discoveries.

## Room-Local Storage and Hauling

Physical item records are the sole source of truth for prototype resources, inventory, collected byproducts, harvested specimen materials, receptacles, residue, and waste. Lab-wide and room-local totals are derived read models rather than mutable storage ledgers. Each record stores its actual tile or container interior, room, quantity, unit mass and volume, form, tags, containing fixture or floor location, stockpile designation, and last observed quantity. Physical availability depends on where the item is, whether its fixture is accessible, and whether an actor can reach an interaction point. Waste still defaults to the Pits because it is a problem to manage, not a clean supply shelf.
...
Collection Bay transfer is also queued. The scientist closes the station path, swaps the filled physical receptacle for an available empty compatible vessel, carries the filled vessel toward Storage, and leaves any apparatus overflow in the Collection Bay to flow into the replacement receptacle. The Stores Collection Stations tab is a read-only station view showing each station's active receptacle, overflow buffer, output, method, and support. Transfer Receptacle lives on the selected station container's contextual commands, keeping the record tab informational while the map selection remains the action surface. Overflow remains apparatus contents rather than collected inventory.

The first black market economy foundation turns collected natural byproducts into illegal trade goods. A run starts with a small set of persistent contacts, each with individual trust, a risk profile, and generated deals. The lab also tracks global black market reputation and money. Selling is not an instant "sell all" button: each sale queues a scientist Black Market task, requires the material to exist in collected byproduct stockpiles, sends the scientist out through the Concealed Exit, and makes the scientist unavailable for other lab work until the trade completes. Suspicion does not rise automatically on every sale; each deal has an exposure chance, and Suspicion rises only if the exchange is observed by authorities. Completed sales record negative byproduct history entries, pay money, improve reputation, improve contact trust, and generate replacement offers. This is intentionally a foundation for later commissions, scams, rare buyers, referrals, delivery failures, blackmail, raids, and broader illegal services.
...
Environmental state is authoritative at the tile level rather than on room labels. Each excavated air tile stores temperature in degrees Celsius, relative humidity, ambient mana density in thaums/m³, and a sparse mixture of exact airborne substance identities and concentrations. Generic Contamination remains only a player-facing aggregate of that mixture. Room and inferred-compartment values are derived weighted summaries, so renaming, splitting, merging, or deleting room designations cannot move air or erase hazards.

Open adjacent tiles exchange heat, humidity, mana, and each airborne substance independently. Exact airborne identities are conserved through diffusion and persist indefinitely unless a physical process settles, decays, absorbs, or removes them. Doors modify exchange according to state, condition, material, effective seal quality, and relevant wards. Natural rock is airtight but conducts heat and mana. Constructed barriers can slowly leak air and humidity according to material porosity and damage, while thermal resistance and arcane permeability govern heat and mana transfer. A breached wall is not passable until destroyed, but attacks and environmental transfer may pass through the breach according to its condition.

Container interiors remain separate environments and exchange only with the exact tile at their physical access location. Geometry supplies interior volume; open state, material porosity, seal quality, condition, and wards determine the transfer rate. Temperature and mana exchange according to material properties, while airborne substances preserve their identities across the boundary. Ordinary UI reports descriptive environmental bands and physical fantasy units. Exact measurements and substance identities are available through debug inspection. Light is now a tile field produced by physical fixtures; electrical charge remains a room-era summary until electrical hazards and stored charge receive their own physical model.

Environmental infrastructure is physical rather than a room-wide toggle. Air ducts, drains, electrical conduits, and mana conduits form separate networks only when their fixture footprints touch or are cardinally adjacent. Devices may share a map tile when they occupy different physical layers, but deconstruction targets the selected fixture's stable ID. Ordinary powered devices can use local fuel, connected electricity, or connected stored mana when their definition supports that source. Fuel-burning devices consume physical Fuel Reagent and add heat and fuel smoke to their own tile or the adjacent room air when wall-mounted. The starter laboratory uses independent primitive devices rather than a central utility plant; most active climate and processing devices begin switched off, while an oil lamp provides initial local light.

Lighting propagates from active lamps across excavated tiles with distance falloff. Solid rock, constructed walls, and closed intact doors block light. Heating changes the heater's exact tile and then spreads through ordinary environmental exchange. Heaters expose broad Cold, Comfortable, and Warm controls until an operational thermostat in the same room permits an exact Celsius target.
...
Natural solid rock has an implicit stone composition with sparse saved ore-bearing deposits. Mining creates local physical rubble rather than immediately adding refined resources to inventory: ordinary rock yields stone rubble, while ore-bearing rock yields both stone and ore fragments. Deconstructing walls, floors, and doors likewise leaves material-tagged rubble on the affected tile. Rubble is visible and persistent but hauling, processing, stacking, obstruction, and stockpile acceptance belong to the later loose-item and labor passes.
...
Future construction, room expansion, damage, sealing, utility maintenance, and creature movement should extend this physical map and work-order model. Detailed room interaction starts from map selection and contextual commands rather than always-visible room cards; room stockpiles, station records, policies, tasks, and historical logs live in their management menus. Construction now records physical excavation and deconstruction rubble and supports buildable first-pass utility networks, but does not yet model rubble hauling or processing, noise, secrecy, additional workers, structural supports, automated utility service work, or fluid flow; those should layer onto persistent tile orders and physical terrain rather than replacing them.
...
Slimes have drive records stored at `slime.ai.drives`. Current drive categories are Hunger, Regrowth, Injury, Stress, Containment, and Division Readiness. Work is not a biological drive. Creature job assignments are player intention tags that help categorize intended use and observed performance; they do not compel an unintelligent slime to act. A satiated or seriously threatened slime can therefore ignore consumptive work even while it remains tagged for that job. Division Readiness is a physiological process rather than a chosen nesting intention: a slime that satisfies the established mass, nutrition, integrity, stress, maturity, and sustained-fullness requirements divides wherever it already is.
...
- Tile lighting and physical first-pass ventilation, drainage, heating, electrical, and mana infrastructure with finite filters, sumps, fuel, feedstock, and mana storage.
...
- Economy workspace with a Black Market menu, persistent contacts, global reputation, money, generated byproduct deals, and queued scientist trade tasks.
