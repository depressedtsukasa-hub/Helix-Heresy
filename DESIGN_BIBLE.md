...
Construction begins from the physical blueprint through a shared designation workflow. Current map modes are Mine, Smooth Floor, Smooth Wall, Cancel Orders, and the reserved Build and Deconstruct modes. Left-click and drag paint the active draft, the draft can be cleared without affecting confirmed work, and confirmation creates a persistent construction order at a Dwarf Fortress-style priority from 1 to 7, where 1 is highest. Build and Deconstruct already have explicit disabled commands and explanations, but their physical results wait for the wall, floor, door, fixture, and material passes rather than inventing temporary abstractions that would need replacement.
