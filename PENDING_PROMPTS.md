...
Slime behavior should be shaped by current biological state. Hunger, stress, body integrity, current mass, maturity, division pressure, containment comfort, environmental fit, pain, fear, and job assignment can all influence what a slime wants to do. These should feed into a priority system that decides whether a slime rests, feeds, wanders, hides, presses against containment, continues work, flees, fights, or stays still.

The system should answer questions like:
- Which existing slime condition stats become AI drives?
- Which drives matter for contained slimes versus loose slimes?
- How does hunger differ from low current mass?
- How does stress change priority selection?
- How does body integrity change behavior?
- How does maturity or division pressure affect behavior?
- How do job assignments suppress, redirect, or compete with natural drives?
- How should drive priority be displayed without exposing exact hidden formulas?

The system should allow simple slimes to behave simply. A starving slime might seek food. An injured slime might avoid danger. A stressed slime might try to hide, freeze, attack, or test containment depending on temperament and traits. A healthy, comfortable slime may remain idle or continue assigned work.

The desired result is a readable drive model that makes slime behavior feel biological and condition-driven rather than random. This should become the backbone for feeding search, escape behavior, habitat response, work autonomy, and combat behavior.

Before coding, discuss which needs exist in the first pass, how they produce priorities, how much is visible to the player, and how drives interact with current slime condition stats.
...
A slime should not magically know everything in the lab. Its behavior should depend on what it can sense or discover. Simple slimes may respond to nearby food, heat, moisture, light, vibration, mana, corpses, contamination, predators, the scientist, or containment stress. Smarter creatures may eventually understand doors, paths, targets, threats, and opportunities more deliberately.

The system should answer questions like:
- What can a slime perceive from inside a container?
- What can a loose slime perceive in its room?
- Can slimes sense through open doors, closed doors, vents, drains, or containers?
- How do element, body consistency, appendages, intelligence, stress, and condition affect perception?
- How do food, corpses, waste, hazards, other creatures, and the scientist become perceived targets?
- Does perception update constantly, periodically, or only when relevant events happen?
- How should the UI show what a slime appears to notice without revealing hidden stats?

Perception should support both laboratory behavior and later combat. A slime might smell carrion, feel heat, detect moisture, sense mana, notice a moving actor, or fail to perceive something outside its container. The same perception model can later support stalking, ambush, fleeing, guarding, and Analyze-related uncertainty.

The desired result is a perception foundation where slime goals are based on sensed local information instead of omniscient global state.

Before coding, discuss perception types, range, container limits, door limits, target memory, performance, and the first useful player-facing readouts.
...
The lab has a physical blueprint, room footprints, doors, object glyphs, movement speeds, and pathfinding. Slimes should be able to use that foundation when they are loose, uncontrolled, assigned to work that requires movement, or responding to drives.

The system should answer questions like:
- When can a slime move on its own?
- How does a slime choose a destination?
- How does movement speed translate into actual travel time?
- How do shape, body consistency, appendages, element, weight, maturity, injury, stress, and current mass affect movement?
- How do doors, containers, blocked cells, room boundaries, and hazards affect pathing?
- How does a slime behave if no path exists?
- How should movement intent and route be displayed?
- How should map occupancy update during slime movement?

This should not make all slimes move like tactical enemies. Some slimes may ooze slowly, pool in place, crawl toward food, drift to damp corners, or fail to cross dry/hot/bright spaces. Others may be fast, climbing, squeezing, or aggressive. Movement should reflect biology and condition.

The desired result is a map-based creature movement foundation where loose or autonomous slimes occupy real space, move over time, and create logistical problems that future feeding, escape, combat, containment, and job systems can build on.

Before coding, discuss movement triggers, pathfinding rules, door handling, speed calculations, occupancy updates, UI readouts, and how to keep the first implementation small.
